...
<Warning>
The manual triggers `POST /issues/from-controls/{orgId}` and `POST /issues/classify`
described on this page have been **removed**. Both now only run automatically
as part of `POST /ingest/{clientOrgId}` — see
[Automated Ingest Pipeline](/flow/00-automated-ingest-pipeline). The rest of
this page still describes the underlying synthesis/classification logic accurately.
</Warning>
...
The worker loads **this run's new controls only** (default
`ISSUES_GENERATION_SCOPE=new_controls`), processes them in batches of
`ISSUES_CONTROLS_BATCH_SIZE` (default 22), and asks the LLM to synthesise
**3–8 issues per batch**, each citing the `control_ids` it was derived from.
Existing controls, issues, and risks are **never deleted** — re-ingesting only
adds what the new documents produce. If `classify_after` is set, a **chained**
classification job is spawned automatically once the issues exist.

```mermaid
flowchart TD
    A[POST issues/from-controls/orgId] --> B[Job: issues_from_controls]
    B --> C[Load this run's new controls]
    C --> D{Batch controls}
...
<Info>
**Incremental & de-duplicated.** Issues are derived from each run's **new
controls**, so overlapping uploads don't re-create the same issue: a deterministic
`source_fingerprint` — a SHA-256 over the org, origin, sorted control ids, and
normalised title, backed by a unique index — suppresses duplicates. Because issues
are **no longer wiped** between runs, risks that were previously auto-promoted from
an issue **stay linked** — the old "wipe and re-derive from all of the org's
controls" behaviour orphaned them. That legacy behaviour is still available via
`ISSUES_GENERATION_SCOPE=all_controls_replace`.
</Info>
...
| --- | --- | --- | --- |
| ~~`POST`~~ | ~~`/issues/from-controls/{orgId}`~~ | — | **Removed** — runs automatically inside `/ingest` |
| ~~`POST`~~ | ~~`/issues/classify`~~ | — | **Removed** — runs automatically inside `/ingest` |
| `GET` | `/issues?client_org_id=…` | Bearer | List issues (`include_classification=true` to embed) |
