...
<Warning>
The manual trigger `POST /control-documents/extract/{orgId}` described on this
page has been **removed**. This stage now only runs automatically as part of
`POST /ingest/{clientOrgId}` — see [Automated Ingest Pipeline](/flow/00-automated-ingest-pipeline).
The rest of this page still describes the underlying extraction logic accurately.
</Warning>
...
bullet / distinct obligation**. A document's segments are extracted
**concurrently** (`EXTRACT_SEGMENT_CONCURRENCY`, default 3); its controls are then
de-duplicated and written in a **single burst once all of that document's segments
finish** — not incrementally per segment. Each document is its **own batch task**,
so several documents' controls land in parallel.
...
flowchart TD
    ...
    G --> H
    H --> I[LLM: extract controls per segment<br/>— EXTRACT_SEGMENT_CONCURRENCY concurrent —]
    I --> J[De-duplicate + assign source_page]
    J --> K[(Persist all controls in one burst<br/>per document)]
    K --> M[Incrementally index only this<br/>document's controls in Milvus<br/>— bulk-embedded —]
    M --> L([Poll job → completed])
```

<Info>
**Incremental vector indexing.** Extraction now indexes **only the new document's
controls** into Milvus (bulk-embedded in one pass), instead of re-embedding the
entire org's control corpus once per document — removing the old
O(documents × whole-corpus) cost as an organisation accumulates controls.
</Info>
...
<Steps>
  ...
  <Step title="Worker reads each PDF">
    ...
    DI (page-batches can run `DI_BATCH_CONCURRENCY` — default 2 — at a time) or to
    **local** PDF text extraction so processing never simply stops.
  </Step>
  ...
  <Step title="Poll and read">
    ...
    `GET /controls?client_org_id=…` — the list grows **one document at a time** as
    each document's batch task finishes (a document's controls appear together, not
    trickled in per segment).
  </Step>
</Steps>
...
| --- | --- | --- | --- |
| ~~`POST`~~ | ~~`/control-documents/extract/{orgId}`~~ | — | **Removed** — runs automatically inside `/ingest` |
...
- **DI timeout** → each Document Intelligence call is bounded by
  `AZURE_DI_TIMEOUT_SECONDS` (default 300), so a stuck call fails fast to the
  fallback instead of hanging the worker.
