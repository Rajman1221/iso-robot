...
<Warning>
The manual trigger `POST /risk-assignments/run` described on this page has been
**removed**. This stage now only runs automatically as the final step of
`POST /ingest/{clientOrgId}`, right after risk tagging, with `auto_apply=true` —
high-confidence owner matches are applied to `risks.owner_user_id` immediately
rather than waiting for analyst approval. Lower-confidence matches still land
as `proposed`/`needs_review` for `POST /risk-assignments/apply-selected`.
...
make ownership official with one approval (unless it was already auto-applied
by the automated pipeline for high-confidence matches).
</Note>
...
| ~~`POST`~~ | ~~`/risk-assignments/run`~~ | — | **Removed** — owner assignment now runs automatically inside `/ingest` |
| `GET` | `/risk-assignments` | Bearer | Read owner recommendations + alternates |
| `POST` | `/risk-assignments/apply-selected` | Bearer | Apply analyst-approved ownership not already auto-applied |
