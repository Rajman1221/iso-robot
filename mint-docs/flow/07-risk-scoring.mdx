...
<Warning>
The manual trigger `POST /risk-scoring/run` described on this page has been
**removed**. This stage now only runs automatically as part of
`POST /ingest/{clientOrgId}` — and now also **auto-promotes** scored issues
into `risks` rows (the automated equivalent of the manual "apply selected
risks" step). See [Automated Ingest Pipeline](/flow/00-automated-ingest-pipeline).
The rest of this page still describes the underlying scoring logic accurately.
</Warning>
...
recommended response from the matrices. Issues are scored in **batches of
`SCORING_BATCH_SIZE`** (default 20) with up to `SCORING_LLM_CONCURRENCY` (5) LLM
calls in flight — bounded-concurrent rather than strictly sequential, so the stage
is roughly N× faster with identical per-issue output.
...
flowchart TD
    ...
    D --> E[LLM: likelihood, consequence, velocity,<br/>risk_type, control effectiveness<br/>— SCORING_LLM_CONCURRENCY concurrent —]
    ...
    K --> L[(Auto-promote scored issues → risks<br/>bulk, idempotent)]
```

<Info>
**Scored after every issue exists, then promoted.** Scoring runs only once **all**
issues from **all** documents in the run have been generated — the batch join is
the barrier — so ratings are computed over the complete set. It then
**auto-promotes** scored issues into `risks` rows in **bulk**; the promotion is
**idempotent**, so a retried or re-queued run never duplicates a risk. Milvus
indexing for each batch is **bulk** (batched embeddings + one upsert), not one
embed-and-upsert per issue.
</Info>

<Tip>
The concurrency here is **asyncio**, not threads or processes: the LLM and
embedding calls are network I/O, so Python's GIL is not the bottleneck —
overlapping the waiting is exactly what speeds scoring up.
</Tip>
...
| --- | --- | --- | --- |
| ~~`POST`~~ | ~~`/risk-scoring/run`~~ | — | **Removed** — runs automatically inside `/ingest` (also auto-promotes to `risks`) |
