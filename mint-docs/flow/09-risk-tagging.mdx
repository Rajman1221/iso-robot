...
<Warning>
The manual trigger `POST /risk-tagging/run` described on this page has been
**removed**. This stage now only runs automatically as part of
`POST /ingest/{clientOrgId}`, with `auto_apply=true` — high-confidence tags are
applied immediately rather than waiting for analyst approval. Lower-confidence
tags still land as `proposed`/`needs_review` for `POST /risk-tagging/apply-selected`.
...
anything becomes official (unless it was already auto-applied by the automated
pipeline for high-confidence matches).
</Note>
...
| ~~`POST`~~ | ~~`/risk-tagging/run`~~ | — | **Removed** — tagging now runs automatically inside `/ingest` |
| `GET` | `/risk-tags` | Bearer | Read recommendations with confidence + rationale |
| `POST` | `/risk-tagging/apply-selected` | Bearer | Apply analyst-approved tags not already auto-applied |
