...
<Info>
Under the hood each stage runs as a **dispatch → batch → finalize** state
machine: item-heavy stages fan out into batches across the worker queues and
make their LLM calls concurrently, then a finalize step aggregates and advances.
This is the default **v2** orchestration (`PIPELINE_V2_ENABLED=true`); the legacy
single-chain path remains behind `PIPELINE_V2_ENABLED=false`.
</Info>
...
```json
{
  ...
  "progress_percent": 72,
  "stage_summary": [
    { "stage": "extract_controls", "status": "completed", "document_count": 3, "failed_count": 0 },
    { "stage": "score_risks", "status": "running", "document_count": 1, "failed_count": 0 }
  ],
  "total_documents": 5,
...
<Info>
These debug jobs now **execute on Celery workers**, via a generic `run_legacy_job`
task routed by job type to the right queue (`LEGACY_JOBS_VIA_CELERY=true`), instead
of running in-process in the API event loop — so they no longer compete with request
handling. The jobs-table status contract (`GET /jobs/{jobId}`) is **unchanged**.
</Info>
...
sequenceDiagram
    ...
    C->>A: POST /controls/extract | /jobs  (start action)
    A->>DB: create job (status=pending)
    A-->>C: { job_id, status: pending }
    A->>W: enqueue run_legacy_job (routed to queue by type)
    W->>DB: status=running
    loop until done
...
They now execute on Celery workers via the shared `run_legacy_job` task
(`LEGACY_JOBS_VIA_CELERY=true`) rather than in the API process.
...
- **LLM calls are hardened**: every chat call retries transient errors (rate
  limits, timeouts, 5xx) with jittered backoff under a per-call timeout, and
  records latency / token / cost metrics. On exhaustion it falls back to the
  deterministic heuristics below where a fallback exists, so the pipeline still
  produces output.
- When the LLM or Document Intelligence is unavailable, the pipeline falls back to
  deterministic heuristics where a fallback exists.
- **Whole batches** retry at the Celery-task level (`CELERY_BATCH_MAX_RETRIES`,
  default 3) and **dead-letter** to `<queue>.dlq` on exhaustion **without stalling
  the run**; `scripts/replay_dlq.py` re-queues them once the cause is fixed.
- A failed run never blocks the queue — a second ingest while a run is active is
  accepted as a **`waiting`** run (`PIPELINE_MAX_QUEUED_RUNS`, default 10) and
  auto-starts when the active one finishes.
